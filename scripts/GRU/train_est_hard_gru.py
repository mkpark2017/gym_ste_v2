...
#            print(action)
        ...
        np_hidden_in = hidden_in.cpu().detach().numpy()
        np_hidden_out = hidden_out.cpu().detach().numpy()
        agent.observe(np_hidden_in, np_hidden_out, reward, obs2, last_action, done)
        print("steps: ", step)
        if step > args.warmup:
            agent.update_policy()
            # Evaluation
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hidden', default=10, type=int, help='hidden num of layer')
...
#    parser.add_argument('--hidden3', default=300, type=int, help='hidden num of third fully connect layer')
    ...
    #warmup 5e5
    parser.add_argument('--rbsize', default=10000000, type=int, help='Replay buffer size, after exceeding this limits, older entries will be replaced by newer ones')
    #repeat memory 1e7
...
#    agent = DDPG_GRU(replay_buffer, nb_states, nb_actions, args)
    agent = DDPG_GRU(nb_states, nb_actions, args)
