...
#        etc_low_state = np.array([-1, -20, 0, -1, 0, 0, 0]) # [wind_direction, wind speed (m/s), last action (only direction), last concentration, concentration (max 100 mg/m^3), last highest conc]
        etc_low_state = np.array([-1, 0, -self.agent_dist, -self.agent_dist, -1, 0, 0]) # [wind_direction, wind speed (m/s), boundary warning (x,y), last action (only direction), last concentration, concentration (max 100 mg/m^3)]
...
#        etc_high_state = np.array([1, 20,  self.max_step,  1, self.conc_max, self.conc_max, self.conc_max])
        etc_high_state = np.array([1, 20,  self.agent_dist, self.agent_dist, 1, self.conc_max, self.conc_max])
    ...
    def _observation(self):
        ...
        etc_state = np.array([float(self.wind_d/math.pi), float(self.wind_s), x_warning, y_warning, float(self.last_action), float(self.last_measure), float(self.gas_measure)])
    ...
    def step(self, action):
        ...
        converge_done = bool(self.cov_val < agent_dist/3)
...
#            rew = self._step_reward()
        ...
        if converge_done: # particle filter is converged
...
#            rew = self._reward_failed()
        ...
        if self.normalization:
            ...
            info = "time step:" + str(self.count_actions) + ", act:" + str(
                    ...
                    round(norm_obs[6]*self.conc_max,2)) + ", particles_x : " + str(
