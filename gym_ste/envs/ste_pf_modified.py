...
class StePFilterModEnv(BasicSteEnv):
...
#        etc_high_state = np.array([100, 100, self.court_lx,  self.court_ly,  self.max_step,  1, self.conc_max, self.conc_max, self.conc_max])
        ...
        self.conc_eps = 0.1 # minimum conc
...
#        self.num_cores = int((multiprocessing.cpu_count())/2)
    ...
    def _pf_gas_conc(self, pos_x, pos_y, source_x, source_y, source_q): # true gas conectration
        avoid_zero = (np.sqrt(pow(source_x - pos_x,2) + pow(source_y - pos_y,2) ) < 1e-10) # to avoid divide by 0
        
        source_x[avoid_zero] += 1e-10
        source_y[avoid_zero] += 1e-10

        dist = np.sqrt(pow((source_x - pos_x), 2) + pow(source_y - pos_y, 2))
        y_n = -(pos_x - source_x)*np.sin(self.wind_mean_phi*math.pi/180)+ \
               (pos_y - source_y)*np.cos(self.wind_mean_phi*math.pi/180)
        lambda_plume = np.sqrt(self.gas_d * self.gas_t / (1 + pow(self.wind_mean_speed,2) * self.gas_t/4/self.gas_d) )
        conc = source_q/(4 * math.pi * self.gas_d * dist) * np.exp(-y_n * self.wind_mean_speed/(2*self.gas_d) - dist/lambda_plume)
        return conc

    def _weight_update(self, i):
        pf_conc = self._pf_gas_conc(self.agent_x, self.agent_y, self.pf_x, self.pf_y, self.pf_q)
#        print(pf_conc)
        pdetSig = np.sqrt( pow((self.gas_measure*self.sensor_sig_m),2) + pow(self.env_sig,2) )
#        print(pdetSig)
        pdetSig_sq = pow(pdetSig, 2)
#        print(pdetSig_sq)
        if pdetSig_sq<1e-100: pdetSig_sq = 1e-100

        p_val = (self.gas_measure - pf_conc)/pdetSig
        p_new = 1/(np.sqrt(2*math.pi)*pdetSig_sq)*np.exp(-pow(p_val,2)/2)
        p_new[p_new < 1e-100] = 1e-100
...
#        for i in range(0,N):
        ...
        for _ in range(1):
            ...
            n_new = self._weight_update(1)
            for i in range(0,N):
#                n_new.append(self._weight_update(i))
...
#                self.pf_q[i] = nXqp[i]
    ...
    def _particle_filter(self):
        pf_concs = []
        Wp_sum = 0
        gauss_new = self._weight_update(1)
        self.Wps = self.Wpnorms * gauss_new
        Wp_sum = sum(self.Wps)

#        for i in range(0,self.pf_num):
#            gauss_new.append(self._weight_update(i))
...
#        etc_state = np.array([float(wind_x), float(wind_y), float(self.last_x), float(self.last_y), float(self.dur_t), float(self.last_action), float(self.last_measure), float(self.gas_measure), float(self.last_highest_conc)])
    ...
    def reset(self):
        ...
        if math.sqrt(pow(self.goal_y - self.agent_y,2) + pow(self.goal_x - self.agent_x,2)) < 0.3*self.court_lx:
            self.reset()
        self.positions.append([self.agent_x, self.agent_y])
        if self.debug:
...
register(
    id='StePFilterModEnv-v0',
    entry_point='gym_ste.envs:StePFilterModEnv',
)
