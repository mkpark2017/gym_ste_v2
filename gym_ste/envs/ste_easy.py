...
class SteEasyEnv(BasicSteEnv):
    ...
    def __init__(self):
        ...
        self.obs_low_state = np.array([-100, -100, 0, 0,  0, -1, 0, 0])
        self.conc_max = 100
        self.obs_high_state = np.array([100, 100, self.court_lx,  self.court_ly,  self.max_step,  1, self.conc_max, self.conc_max])
    ...
    def _observation(self):
        ...
        return np.array([float(wind_x), float(wind_y), float(self.last_x), float(self.last_y), float(self.dur_t), float(self.last_action), float(self.gas_measure), float(self.last_highest_conc)])
    ...
    def step(self, action):
        ...
        if self.normalization:
           ...
           info = "time step:" + str(self.count_actions) + ", act:" + str(
                   ...
                   round(norm_obs[1],2)) + "), last pos: (" + str(round(norm_obs[2]*self.court_ly,2)) + ", " + str(
                   round(norm_obs[3]*self.court_ly,2)) + "), dur_t from last capture: " + str(round(norm_obs[4],2)) + ", last action: " + str(
                   round(float(norm_obs[5])*180,2)) + ", conc" + str(round(norm_obs[6]*self.conc_max,2)) + ", last highest conc" + str(
                   round(norm_obs[7]*self.conc_max,2)) + ", rew:" + str(rew) + ", current pos: (" + str(round(self.agent_x,2)) + "," + str(
           ...
           info = "time step:" + str(self.count_actions) + ", act:" + str(
                   ...
                   round(float(obs[5])*180,2)) + ", conc" + str(round(obs[6],2)) + ", last highest conc" + str(
                   round(obs[7],2)) + ", rew:" + str(rew) + ", current pos: (" + str(round(self.agent_x,2)) + "," + str(
                   round(self.agent_y,2)) + ")", "goal pos: (" + str(self.goal_x) + "," + str(self.goal_y) + "), done: " + str(done)
...
register(
    id='SteEasyEnv-v0',
    entry_point='gym_ste.envs:SteEasyEnv',
)
