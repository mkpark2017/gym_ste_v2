    ...
    def __init__(self):
        ...
        self.obs_low_state = np.array([-1, 0,        # wind_direction, wind speed (m/s)
                                       ...
                                       -1,           # last action
                                        ...
                                        0, 0, 0, 0, 0, 0]) # mean_x, mean_y, mean_q, cov_x, cov_y, cov_q
        self.obs_high_state = np.array([1, 20,
                                        self.max_step,
                                        self.court_lx, self.court_ly,
                                        1,
                                        self.conc_max, self.conc_max, self.conc_max,
                                        self.court_lx, self.court_ly, self.max_q, pow(self.court_lx,2), pow(self.court_ly,2), pow(self.max_q, 2) ])
...
#        x_warning, y_warning = self._boundary_warning_sensor()
        ...
        obs = np.array([float(self.wind_d/math.pi), float(self.wind_s),
                        ...
                        float(mean_x), float(mean_y), float(mean_q), float(self.CovXxp), float(self.CovXyp), float(self.CovXqp)])
    ...
    def step(self, action):
        ...
        pf_center = np.array([np.sum(self.pf_x*self.Wpnorms), np.sum(self.pf_y*self.Wpnorms)])
        mean_q = np.sum(self.pf_q * self.Wpnorms)
